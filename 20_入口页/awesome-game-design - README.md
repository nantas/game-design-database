---
title: "awesome-game-design - README"
type: entry_page
source: "awesome-game-design"
url: "https://github.com/Roobyx/awesome-game-design"
status: active
tags: [game-design, entry-page, github, awesome-list]
---

# awesome-game-design - README

## 页面摘要

- 来源页：[[10_来源/awesome-game-design|awesome-game-design]]
- 页面性质：GitHub README 型索引页；本页本身不是直接设计模式证据。
- 当前高价值区块：`Finished games GDDs`、`Postmortems`、`Learning materials`。
- 当前降级区块：`Templates and examples` 仅作补充参考；`GDD Creation Tools`、`Game design tools`、`Game development` 暂不进入设计模式主队列。
- 冻结依据：按 2026-03-09 抓取到的当前 README 结构整理。

## 候选链接

### 高价值候选主队列

- [ ] Monaco - https://www.facebook.com/notes/monaco/the-original-monaco-game-design-from-2003/496033541995/ - 类别：Finished games GDD - 优先级：P1 - 状态：blocked - 原因：当前落在 Facebook 社交页，缺少稳定可抽取正文，不适合仓库现阶段导入
- [ ] Id Doom - https://5years.doomworld.com/doombible/doombible.pdf - 类别：Finished games GDD - 优先级：P1 - 状态：blocked - 原因：当前请求返回 Cloudflare challenge 页面，原始 PDF 直链不可稳定访问
- [ ] Prince of Persia 2 - https://www.popot.org/documentation/documents/1991-08-08_PoP2_Design_Bible.pdf - 类别：Finished games GDD - 优先级：P1 - 状态：ready-for-next-step - 原因：当前为可直接访问的公开 PDF，适合按正常网页卡流程处理
- [ ] Condor's Diablo 1 - http://www.graybeardgames.com/download/diablo_pitch.pdf - 类别：Finished games GDD - 优先级：P1 - 状态：ready-for-next-step - 原因：当前为可直接访问的公开 PDF，适合按正常网页卡流程处理
- [ ] Grim Fandango - http://gameshelf.jmac.org/2008/11/13/GrimPuzzleDoc_small.pdf - 类别：Finished games GDD - 优先级：P1 - 状态：ready-for-next-step - 原因：当前为可直接访问的公开 PDF，适合按正常网页卡流程处理
- [ ] Balatro Timeline - https://localthunk.com/blog/balatro-timeline-3aarh - 类别：Postmortems - 优先级：P2
- [ ] System Shock 2 - https://web.archive.org/web/20190325062431/http://www.gamasutra.com/view/feature/3406/postmortem_irrational_games_.php - 类别：Postmortems - 优先级：P2
- [ ] Deus Ex - https://web.archive.org/web/20190325062431/http://www.gamasutra.com/view/feature/3114/postmortem_ion_storms_deus_ex.php - 类别：Postmortems - 优先级：P2
- [ ] Diablo II - https://web.archive.org/web/20190325062431/http://www.gamasutra.com/view/feature/3124/postmortem_blizzards_diablo_ii.php - 类别：Postmortems - 优先级：P2
- [ ] BioShock - https://web.archive.org/web/20190325062431/http://www.gamasutra.com/view/feature/3774/postmortem_2k_boston2k_.php - 类别：Postmortems - 优先级：P2

### 理论 / 框架队列

- [ ] Lost Garden - https://lostgarden.home.blog/ - 类别：Learning materials - 优先级：T1
- [ ] The Door Problem - https://lizengland.com/blog/2014/04/the-door-problem/ - 类别：Learning materials - 优先级：T1
- [ ] Designer Notes - https://www.designer-notes.com/ - 类别：Learning materials - 优先级：T2
- [ ] Game Design Patterns Wiki - https://virt10.itu.chalmers.se/index.php/Main_Page - 类别：Learning materials - 优先级：T2

### 暂不纳入本轮

- [ ] Generic example 1 - https://drive.google.com/file/d/1-yiF2Pq-OgJaTXsMAQbIckoDzGINz26O/view - 原因：GDD 模板/示例，不是设计模式证据优先项
- [ ] Dundoc - https://www.dundoc.com/ - 原因：GDD Creation Tools，属于工作流工具
- [ ] Arrow - https://github.com/mhgolkar/Arrow - 原因：Narrative Design Tool，当前不进入模式证据主队列
- [ ] 3D Math Primer for Graphics and Game Development - https://gamemath.com/ - 原因：泛开发学习资源，不属于本轮游戏设计模式主目标
- [ ] GDC Postmortems - https://youtube.com/playlist?list=PLNu0OXEmMXnA46j4CDZLVsVj-ysmWwW6s - 原因：视频播放列表，缺少稳定正文载体

## 后续处理

- 本轮只把 README 作为入口页和分流面板使用，不把该 awesome 页本身推进为 `30_网页卡/` 证据页。
- 优先从 `P1` 的 finished GDD 文档开始，随后处理 `P2` 的文本型 postmortem，再处理 `T1/T2` 的理论与框架入口。
- 具体分流规则、处理顺序与队列定义见 [[50_专题索引/awesome-game-design - 初始分诊策略|awesome-game-design - 初始分诊策略]]。

## 上下文参考

- [[10_来源/awesome-game-design|awesome-game-design]]
- [[50_专题索引/awesome-game-design - 初始分诊策略|awesome-game-design - 初始分诊策略]]
